...
const getRateLimiter = (apiService, limit) => {
  ...
  const send = requestId => {
        // console.log(`sendCall:${++sendCall}`);
        receivedRequests.push(requestId);
        return new Promise((resolve, reject) => {
          const throttleRequest = (requestID) => {
            apiService(requestID).then(s => {
              runningRequests--;
              if(runningRequests < limit-1 && receivedRequests.length > 0) {
                runningRequests++;
                throttleRequest(receivedRequests.shift());
              }
              resolve(s);
            }, f => {
              //TODO
          ...
          if(runningRequests < limit && receivedRequests.length > 0) {
            runningRequests++;
            throttleRequest(receivedRequests.shift());
          }
      });
